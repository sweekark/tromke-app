...
- (void)setTitleAndSubtitleOutsideDistance:(BOOL)outside {
	if (outside) {
        ...
        if ([self.annotationObject[POST_TYPE] isEqualToString:POST_TYPE_ASK]) {
            self.title = [NSString stringWithFormat:@"%@ asking question", /*postedUser[USER_DISPLAY_NAME]*/ [TUtility getDisplayNameForUser:postedUser]];
        } else if ([self.annotationObject[POST_TYPE] isEqualToString:POST_TYPE_IMAGE]) {
            self.title = [NSString stringWithFormat:@"%@ posted image", /*postedUser[USER_DISPLAY_NAME]*/[TUtility getDisplayNameForUser:postedUser]];
        } else if ([self.annotationObject[POST_TYPE] isEqualToString:POST_TYPE_STICKER]) {
            self.title = [NSString stringWithFormat:@"%@ posted sticker", /*postedUser[USER_DISPLAY_NAME]*/[TUtility getDisplayNameForUser:postedUser]];
        }
