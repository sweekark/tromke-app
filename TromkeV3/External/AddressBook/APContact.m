...
- (NSString *)contactPhones
{
    if (self.phonesWithLabels.count > 0)
    {
        //        NSMutableString *result = [[NSMutableString alloc] init];
        NSString* result;
        for (APPhoneWithLabel *phoneWithLabel in self.phonesWithLabels)
        {
            //            NSString *string = phoneWithLabel.label.length == 0 ? phoneWithLabel.phone :
            //            [NSString stringWithFormat:@"%@ (%@)", phoneWithLabel.phone, phoneWithLabel.label];
            //            [result appendFormat:@"%@, ", string];
            result = phoneWithLabel.phone;
            break;
        }
...
- (NSDictionary*)dictionary {
    return @{CONTACT_PHONE :[self contactPhones], CONTACT_FIRSTNAME : self.firstName ? self.firstName : @"", CONTACT_LASTNAME : self.lastName ? self.lastName : @""};
}
